...
describe('MyDocuments', () => {
    ...
    it('charge et affiche la liste de documents', async () => {
        const docs = [
            { name: 'a.pdf', url: 'urlA', created: '2023-01-01T00:00:00Z', leaseId: 1 },
            { name: 'b.pdf', url: 'urlB', created: '2023-02-02T00:00:00Z', leaseId: 2 },
        ];
        mockedFetch.mockResolvedValueOnce(docs);
        render(_jsx(MyDocuments, { jwt: jwt }));
    ...
    it('affiche une erreur si deleteDocument échoue', async () => {
        mockedFetch
            ...
            { name: 'x.pdf', url: 'urlX', created: '2023-03-03T00:00:00Z', leaseId: 1 }
        ])
            .mockResolvedValueOnce([
            { name: 'x.pdf', url: 'urlX', created: '2023-03-03T00:00:00Z', leaseId: 1 }
        ]);
