...
const mockUseLeaseActions = useLeaseActions;
beforeEach(() => {
    vi.clearAllMocks();
...
describe("LeaseForm", () => {
    ...
    it("submits createLease on submit when no lease", async () => {
        ...
        const onClose = vi.fn();
        render(_jsx(LeaseForm, { lease: null, onClose: onClose }));
    ...
    it("submits updateLease when lease is provided", async () => {
        ...
        const onClose = vi.fn();
        render(_jsx(LeaseForm, { lease: mockLease, onClose: onClose }));
