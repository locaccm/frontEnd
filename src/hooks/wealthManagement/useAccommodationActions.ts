...
import { Accommodation, AccommodationPayload } from "../../types/wealthManagement/wealthManagement.js";
...
export const useAccommodationActions = () => {
  ...
  const fetchAccommodations = useCallback(async (): Promise<Accommodation[] | null> => {
  ...
  const createAccommodation = useCallback(async (payload: AccommodationPayload): Promise<boolean> => {
    const userId = getUserId();
    if (!userId) {
      setError("Utilisateur non connecté.");
      return false;
  ...
  const deleteAccommodation = useCallback(async (id: number): Promise<boolean> => {
    const userId = getUserId();
    if (!userId) {
      setError("Utilisateur non connecté.");
      return false;
